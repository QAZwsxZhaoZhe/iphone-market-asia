...
function finish(notice: string, error = false): never {
  ...
  redirect(`/m/orders?${query.toString()}`);
}
...
export async function fulfillOrderAction(formData: FormData): Promise<void> {
  ...
  try {
    await api.fulfillOrder(orderId, status, text(formData, "note"));
...
export async function refundOrderAction(formData: FormData): Promise<void> {
  ...
  const idempotencyKey =
    text(formData, "idempotency_key") || `refund:${orderId}:${randomUUID()}`;
  try {
    const refund = await api.refundOrder(orderId, reason, idempotencyKey);
    finish(
